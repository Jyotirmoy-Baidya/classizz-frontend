...
const ClassBlock = ({ content }) => {
    return (
        <NavLink to={`/cls/a/${content.code}`} className={`h-32 md:h-36 w-auto min-w-[240px] max-w-[380px] p-6 md:p-6 border-[1px] ${colors[content.c].bg_color} ${colors[content.c].border_color}  hover:border-[2.5px] cursor-pointer text-black dark:text-slate-700 shrink-0 flex justify-between rounded-md`} >
            ...
            <div className="text-xl flex flex-col items-center gap-2 stundent-count">
                <div className="text-2xl md:text-4xl text-slate-700" ><MdGroups3 /></div>
                <div className="text-sm md:text-base font-medium tracking-wider">{content.noOfStudents}</div>
            </div>
        </NavLink>
    )
}
