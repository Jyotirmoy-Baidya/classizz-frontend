...
import { color, quizes } from '../../constants';
                                ...
                                <div className={`p-3 ps-8 h-14 cursor-pointer flex gap-4 items-center rounded-md border-[3px] border-slate-200 relative quiz-block ${active === i + 1 && 'shadow-lg active z-30'} ${i == 0 && 'quiz-live'} overflow-hidden`} onClick={() => { setActive(i + 1); setQuizPop(true) }}>
                                    ...
                                    <div className='text-lg'>{ele.quizName}</div>
                                    {
                                        ele.subjects?.map((sub, i) => (
                                            <div className={`text-xs py-1 px-2 ${color[sub.color].bg} ${color[sub.color].border} border rounded-full`}>{sub.name}</div>
                                        ))
                                    }
                ...
                quizPop === true ? (
                    <div className='fixed h-screen w-screen left-0 right-0 z-30 flex justify-center items-center bg-slate-400 bg-opacity-30'>
                        <div className={`${active != 0 && 'active'} h-[90%] w-1/3 flex flex-col bg-blue-50 shadow shadow-slate-700 rounded-md relative`}>
                            ...
                            <div className='p-4 flex flex-col items-center gap-5 z-10'>
                                <div className='py-4 text-2xl text-white text-center tracking-wide font-medium uppercase quiz-heading'>Quiz Details</div>
                            ...
                            <span className='rounded-b-3xl h-[23%] bg-blue-300 absolute top-0 shadow-lg w-full'></span>
                            {
                                quizActive ?
                                    <div className='h-full'>
                                        <div className='grid grid-cols-2 gap-4 p-4 quiz-body'>
                                            ...
                                            <div className='flex items-center gap-2'>Status: <span className={`${'easy' == 'easy' && 'bg-green-400'} py-1 px-3 rounded-full text-xs flex items-center`}>Active</span></div>
                                        ...
                                        <div className='flex flex-col h-[48%] my-2 mx-3'>
                                            <div>Students</div>
                                    ...
                                    <div className='h-full w-full flex flex-col relative'>
                                        <div className='text-lg w-fit p-3 border-[1px] border-red-500 rounded quiz-not-started-alert'>Quiz is not yet started</div>
                                        <div className='ms-14'>will start from 22:00</div>
                                    </div>
                            }
                        </div>
                    </div>) : <></>
